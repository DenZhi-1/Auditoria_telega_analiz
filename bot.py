    ...
    try:
        ...
        if not group_info:
            await message.answer(
                ...
                "2. Убедиться, что группа открыта\n"
                "3. Использовать команду /test_vk для проверки подключения"
            )
            return
...
async def cmd_test_vk(message: Message):
    ...
    try:
        ...
        await message.answer(f"❌ <b>Критическая ошибка диагностики:</b>\n\n{str(e)[:200]}")
            ...
            report += "2. Ограничения приложения VK\n"
            report += "3. Проблемы с сетью или блокировки\n"
            report += "4. Превышение лимитов API\n\n"
